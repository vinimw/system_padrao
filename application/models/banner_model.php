...
class Banner_Model extends CI_model {
	...
	public function __construct(){
		...
		define('MEXERBANCO', 'wb_banner');
		define('IDALTERA', 'banner_id');
	...
	public function get ($condicao = array(), $primeiraLinha = FALSE, $pagina = 0){
		...
		$this->db->select('banner_id,banner_nome,banner_titulo,banner_ordem,banner_descricao,banner_dataEntra,banner_dataSai,banner_imagem');
		$this->db->where($condicao);
        $this->db->order_by("banner_ordem", "ASC"); 
...
//        $resize = $this->image_lib->resize();
	...
	public function cadastra($itens){
		...
		$add_foto = $this->do_upload($itens['banner_id']);
		if ($add_foto) {
                $itens['banner_imagem'] = $add_foto['file_name'];
            } else {
	...
	public function update($itens, $idusuario){
        ...
        if ($add_foto) {
            if(isset($add_foto['file_name'])){
                ...
                $itens['banner_imagem'] =  $itens['imagem_antiga'];
            }
        }
